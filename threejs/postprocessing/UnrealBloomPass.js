	...
	class UnrealBloomPass extends THREE.Pass {
		...
		constructor( resolution, strength, radius, threshold ) {
			...
			this.materialCopy = new THREE.ShaderMaterial( {
				...
				// PATCHED: stock config was blending:AdditiveBlending, which uses the
				// same (src-alpha, one) factors for alpha as for RGB. This pass's own
				// bloom texture has alpha forced to 1.0 throughout (see the blur/
				// composite shaders below), so that additive alpha blend pushed
				// *destination* alpha to ~1 across the whole screen quad — not just
				// where bloom is visible — silently destroying a transparent
				// renderer's alpha (needed for our desktop-overlay window).
				// CustomBlending with zero/one alpha factors keeps the additive RGB
				// glow but leaves destination alpha untouched.
